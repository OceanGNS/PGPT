function [ CORR_DOXY ] = apply_tau( MTIME, PRES, DOXY,tau2, varargin )
...
%     tau(m) = nanmin(time_constants(rmsd == nanmin(rmsd)));
    [d1,c1,d2,c2]=corr_DOXY( [ profile1(index1) ; depth1(index1) ; time1(index1) ],...
                           [ profile2(index2) ; depth2(index2) ; time2(index2) ],...
                               tau(m))  ;
    if length(c1)>5 && length(c2)>5

        CORR_DOXY(m,index1)=mean_interp(d1,c1,depth1(index1));
        CORR_DOXY(m+1,index2)=mean_interp(d2,c2,depth2(index2));
    else 
        CORR_DOXY(m,index1)=DOXY(m,index1);
        CORR_DOXY(m+1,index2)=DOXY(m+1,index2);
    end
...
function [d1,c1,d2,c2] = corr_DOXY(P1, P2, tau, z)
    % correct each profile
    corr1 = correct_oxygen_profile(P1(3,:), P1(1,:), tau);
    corr2 = correct_oxygen_profile(P2(3,:), P2(1,:), tau);
%     clean up unique points for interpolation
