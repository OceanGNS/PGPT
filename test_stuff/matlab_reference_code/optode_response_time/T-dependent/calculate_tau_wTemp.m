...
for m=1:M-1
    ...
    if isempty(rmsd(~isnan(rmsd)))
        rmsd = 0*time_constants+1;
        rmsd(1)=0;
        size(rmsd)
        size(time_constants)
        m
...
in=dlmread('T_lL_tau_3830_4330.dat'); lL=in(1,2:end);T=in(2:end,1);tau100=in(2:end,2:end);
clear in
[lL,TEMP_LUT]=meshgrid(lL,T);
