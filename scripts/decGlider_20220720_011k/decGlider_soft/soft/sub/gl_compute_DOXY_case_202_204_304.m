...
% (C1PHASE_DOXY, C2PHASE_DOXY and TEMP_DOXY) according to Case 202_204_304
% specifications.
%
% SYNTAX :
%  [o_doxyValues] = gl_compute_DOXY_case_202_204_304(a_timeValues, ...
%    a_ctdValues, a_tPhaseDoxyValues, a_tempDoxyValues, ...
%    a_timeGps, a_latitudeGps, a_longitudeGps)
%
% INPUT PARAMETERS :
%   a_timeValues       : time measurements
%   a_ctdValues        : CTD measurements
%   a_tPhaseDoxyValues : TPHASE_DOXY measurements
...
% ------------------------------------------------------------------------------
% RELEASES :
%   03/08/2019 - RNU - creation
% ------------------------------------------------------------------------------
function [o_doxyValues] = gl_compute_DOXY_case_202_204_304(a_timeValues, ...
   a_ctdValues, a_tPhaseDoxyValues, a_tempDoxyValues, ...
   a_timeGps, a_latitudeGps, a_longitudeGps)

% output parameters initialization
o_doxyValues = nan(size(a_tPhaseDoxyValues));


inputDoxyNoDef = find(~isnan(a_tPhaseDoxyValues) & ~isnan(a_tempDoxyValues));
if (~isempty(inputDoxyNoDef))
   ...
   if (~isempty(ctdIntData))
      ...
      if (~isempty(latIntGps))
         ...
         % compute DOXY
         o_doxyValues(inputDoxyNoDef) = compute_DOXY_202_204_304( ...
            a_tPhaseDoxyValues(inputDoxyNoDef), ...
...
%
% SYNTAX :
%  [o_doxyValues] = compute_DOXY_202_204_304( ...
%    a_tPhaseDoxyValues, a_tempDoxyValues, ...
...
%   03/08/2019 - RNU - creation
% ------------------------------------------------------------------------------
function [o_doxyValues] = compute_DOXY_202_204_304( ...
   a_tPhaseDoxyValues, a_tempDoxyValues, ...
   a_presValues, a_tempValues, a_psalValues, ...
   a_latitude, a_longitude)
...
if (isempty(a_tPhaseDoxyValues) || isempty(a_tempDoxyValues))
   return
end
