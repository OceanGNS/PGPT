% ------------------------------------------------------------------------------
% Generate the rules file to check the EGO 1.3 format with the Coriolis Java
% checker.
%
% SYNTAX :
%  gl_generate_ego_1_4_checker_rules_file
%
% INPUT PARAMETERS :
%
% OUTPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   08/26/2020 - RNU - creation
% ------------------------------------------------------------------------------
function gl_generate_ego_1_4_checker_rules_file
...
% PARAMETER_FILE_PATH_NAME = 'C:\Users\jprannou\_RNU\Glider\soft\util\ref_lists\argo-parameters-list-core-and-b_CS_20171020.txt';
% PARAMETER_FILE_PATH_NAME = 'C:\Users\jprannou\_RNU\Glider\soft\util\ref_lists\argo-parameters-list-core-and-b_CS_20180129.txt';
% PARAMETER_FILE_PATH_NAME = 'C:\Users\jprannou\_RNU\Glider\soft\util\ref_lists\argo-parameters-list-core-and-b_20210708_AND_glider_specific_parameters_list_20210906.txt';
% PARAMETER_FILE_PATH_NAME = 'C:\Users\jprannou\_RNU\Glider\soft\util\ref_lists\argo-parameters-list-core-and-b_20210708_AND_glider_specific_parameters_list_20220225.txt';
PARAMETER_FILE_PATH_NAME = 'C:\Users\jprannou\_RNU\Glider\soft\util\ref_lists\argo-parameters-list-core-and-b_20210708_AND_glider_specific_parameters_list_20220311.txt';

% output rules file directory
RULES_FILE_OUTPUT_DIR = 'C:\Users\jprannou\_RNU\Checker_Java_Co\checker_20181129\netcdf-checker-1.15\resources\RULES\';

% templates of the rules file to use
% TEMPLATE_RULES_FILE_PATH_NAME = 'C:\Users\jprannou\_RNU\Glider\soft\util\ref_lists\EGO_V1.3_TEMPLATE_20180829.xml';
% TEMPLATE_RULES_FILE_PATH_NAME = 'C:\Users\jprannou\_RNU\Glider\soft\util\ref_lists\EGO_V1.3_TEMPLATE_20190307.xml';
% TEMPLATE_RULES_FILE_PATH_NAME = 'C:\Users\jprannou\_RNU\Glider\soft\util\ref_lists\EGO_V1.4_TEMPLATE_20200826.xml';
% TEMPLATE_RULES_FILE_PATH_NAME = 'C:\Users\jprannou\_RNU\Glider\soft\util\ref_lists\EGO_V1.4_TEMPLATE_20211011.xml';
TEMPLATE_RULES_FILE_PATH_NAME = 'C:\Users\jprannou\_RNU\Glider\soft\util\ref_lists\EGO_V1.4_TEMPLATE_20220225.xml';
...
for idP = 1:length(paramDataStruct)
   ...
   var.core_bio_intermediate_core_intermediate_bio = paramStruct.core_bio_intermediate_core_intermediate_bio;
   
   tabVar{end+1} = var;
   
   %    if (paramStruct.core_bio_intermediate_core_intermediate_bio ~= 'i')
   %
   %       varAdj = var;
...
attCoordinates = '\t\t\t\t<OPTIONAL_ATTRIBUTE NAME="coordinates" PATTERN="TIME LATITUDE LONGITUDE PRES"/>\n';
...
for idV = 1:length(a_tabVar)
   ...
   for idLoop = 1:3
      ...
      if (idLoop == 1)
         varSuffix = '';
      elseif (idLoop == 2)
         if (var.core_bio_intermediate_core_intermediate_bio(1) == 'i')
            continue
         end
         varSuffix = '_ADJUSTED';
      else
         ...
         varSuffix = '_ADJUSTED_ERROR';
      end
      
      if ((idLoop == 1) || (idLoop == 2))

         % <PARAM> & <PARAM>_ADJUSTED
         
         varStr = sprintf(headerPattern, [var.name varSuffix], var.type);
         ...
         % not mandatory attributes: print only attributes with provided value
         ...
         % <PARAM>_QC & <PARAM>_ADJUSTED_QC
         var = a_tabVar{idV};
         
         varStr = sprintf(headerPattern, [var.name varSuffix '_QC'], 'byte');
         varStr = [varStr sprintf(dimPattern)];
         varStr = [varStr sprintf(attWhithValueMandPattern, 'long_name', 'Quality flag')];
         varStr = [varStr sprintf(attWhithValueMandPattern, 'conventions', 'EGO reference table 2.1')];
         ...
         % <PARAM>_ADJUSTED_ERRO
         var = a_tabVar{idV};
         
         varStr = sprintf(headerPattern, [var.name '_ADJUSTED_ERROR'], var.type);
         varStr = [varStr sprintf(dimPattern)];
         varStr = [varStr sprintf(attWhithValueMandPattern, 'long_name', 'Contains the error on the adjusted values as determined by the delayed mode QC process')];
