...
def process_and_save_data(data, filename, gliders_db, attrs):
	...
	if all(k in glider_data for k in ['sci_water_cond', 'sci_water_temp', 'sci_water_pressure']):
		data[['salinity', 'absolute_salinity']] = c2salinity(glider_data['sci_water_cond'].to_numpy(), glider_data['sci_water_temp'].to_numpy(), glider_data['sci_water_pressure'].to_numpy(), glider_data['m_gps_lon'].to_numpy(), glider_data['m_gps_lat'].to_numpy())
		data[['conservative_temperature', 'density']] = stp2ct_density(data['absolute_salinity'].to_numpy(), glider_data['sci_water_temp'].to_numpy(), glider_data['sci_water_pressure'].to_numpy())
...
def main(args):
	...
	for file_arg in [args.filename, args.gliders_db, args.attrs]:
