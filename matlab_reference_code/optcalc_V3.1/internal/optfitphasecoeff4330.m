function out=optfitphasecoeff4330(temp,TCphase,foilcoef,phasecoef0,oxygen,sal,P_atm,oxygenstd,weightedflag,P_dbar,pcfactor)
% function out=optfitphasecoeff4330(temp,TCphase,foilcoef,phasecoef0,oxygen,sal,P_atm,oxygenstd,weightedflag,P_dbar,pcfactor)
% fit phase coefficients of internal aanderaa calculation according to
% measurements (temp, TCphase, sal, atmospheric pressure, hydrostatic 
% pressure) and winkler oxygen values (oxygen, oxygenstd)
...
if nargin<7
    P_atm=ones(shape,1).*1013.25; % mbar
else
    P_atm=P_atm(:);
...
if weightedflag
    % weighted
    X=[temp phase zeros(shape,1) wpO2 1+oxygenstd];
else
    % uniform weights
    X=[temp phase zeros(shape,1) wpO2 ones(shape,1)];
end

% add foilcoefficients as separate variable
% (expand X if necessary)
i=size(X,1);
X(1:21,3)=foilcoef(:); % insert foil coefficients artificially
%X(22,3)=i; % insert number of "real" entries artificially
if i<21
    X(i+1:21,1:2)=NaN;
    X(i+1:21,4:5)=NaN;
elseif i>21
...
% do actual optimisation (of pO2)
[phasecoef,r,J,COVB,mse]=nlinfit(X,zeros(length(X),1),@applyphasecoeff4330,phasecoef0);
% interpret result
...
pO2fit=applyphasecoeff4330(phasecoef(:,1),X(:,1:3));
%shrink calculation output to actual data size
pO2fit=pO2fit(1:i,:);
out.rpO2=wpO2-pO2fit;
out.msepO2=sum(out.rpO2.^2)./length(out.rpO2);
...
plotO=optcalcO2(plotT(:),plotD,foilcoef,'5x5b');
...
functionstring={'Phase coefficient fit; 5x5b model','', 'CalPhase = \Pi_i^n c_i\cdotTCPhase^n',['degree n = ' num2str(degree) weightstring]};
...
subplot(1,5,5)
set(gca,'Visible','Off')
thandle=text(0,.5,[{'Optode 4330 #183','4330 cal'} functionstring ,{['RMSE = ' num2str(sqrt(out.mse),'%6.2f') ' \mumol/l  (N = ' num2str(length(oxygen)) ')'],''}, phasecoef0string phasecoefstring]);

% finally set renderer of figure to painters (not zbuffer)
set(fha,'Renderer','painters')

function F=applyphasecoeff4330(phasecoef,X)
% function ox=applyphasecoeff4330(phasecoef,[temp TCphase foilcoef oxygen weights])
[n,m]=size(X);

t=X(:,1);
Calphase=polyval(flipud(phasecoef),X(:,2));
foilcoef=X(1:21,3);
...
pO=w.*(optodefun5x5b(foilcoef,[t Calphase zeros(n,1) ones(n,1)]));
F=pO-w.*OW;