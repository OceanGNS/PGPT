function F=optodefunUchidasimple(beta,x)
% function F=optodefunUchidasimple(beta,x)
% Stern-Volmer inspired BPhase->pO2 calculation
% beta - 5 coefficients following Stern-Volmer equation-derived
...
%O=w.*(((1+beta(4-1).*t)./(beta(5-1)+beta(6-1).*p) -1)./(beta(1)+beta(2).*t));
%O=w.*(((1+beta(4-1).*t)./(beta(6-2).*p) -1)./(beta(1)+beta(2).*t));
