...
timeGpsFillVal = netcdf.getAtt(fCdf, netcdf.inqVarID(fCdf, 'TIME_GPS'), '_FillValue');
timeGps(timeGps == timeGpsFillVal) = [];
netcdf.close(fCdf);
...
while (~stop)
   ...
   startId = stopId;
   stopId = find(~isnan(time_GTIVT_tmp) & (time_GTIVT_tmp > timeGps(timeGpsId)), 1, 'first');
   timeGpsId = timeGpsId + 1;
   while ((timeGpsId <= length(timeGps)) && (length(startId:stopId) < 1000))
      stopId = find(~isnan(time_GTIVT_tmp) & (time_GTIVT_tmp > timeGps(timeGpsId)), 1, 'first');
      timeGpsId = timeGpsId + 1;
   end
   if (timeGpsId > length(timeGps))
