...
% ------------------------------------------------------------------------------
function nc_ego_adj_2_csv(varargin)
...
logFile = [DIR_LOG_FILE '/' 'nc_ego_adj_2_csv_' datestr(now, 'yyyymmddTHHMMSS') '.log'];
diary(logFile);
tic;
...
if (isempty(dataToProcessDir))
   ...
   for dirNum = 1:length(dirInfo)
      if ~(strcmp(dirInfo(dirNum).name, '.') || strcmp(dirInfo(dirNum).name, '..'))
         ...
         nc_ego_adj_2_csv_file([DIR_INPUT_NC_FILES '/' dirName]);
      end
   end
else
   % convert the data of this deployment
   nc_ego_adj_2_csv_file(dataToProcessDir);
end
...
%
% SYNTAX :
%  nc_ego_adj_2_csv_file(a_dirName)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function nc_ego_adj_2_csv_file(a_dirName)
...
for idF = 1:length(ncFiles)
   ...
   for id = 1:length(paramList)
      for idLoop = 1:2 % for <PARAM> and <PARAM>_ADJUSTED
         paramName = paramList{id};
         if (idLoop == 2)
            paramName = [paramName '_ADJUSTED'];
            if (isempty(gl_get_data_from_name(paramName, measData)))
               continue
            end
         end
         paramListData{end+1} = paramName;
         fprintf(fidOut, '; %s; Qc', paramName);
         paramData = cat(2, paramData, gl_get_data_from_name(paramName, measData));
         paramDataQc = cat(2, paramDataQc, gl_get_data_from_name([paramName '_QC'], measData));
      end
   end
   fprintf(fidOut, '\n');
   fprintf(fidOut, '-------\n');
   ...
   for idJ = 1:length(juld)
      ...
      for idM = 1:length(paramListData)
