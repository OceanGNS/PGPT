...
%
% SYNTAX :
%   nc_remove_attribute or 
%   nc_remove_attribute('data', 'crate_mooset00_38')
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   08/17/2021 - RNU - V 02aa: remove empty 'valid_min' and 'valid_max'
%                              attributes.
% ------------------------------------------------------------------------------
function nc_remove_attribute(varargin)
...
g_couf_ncUpdateEGOFormatVersion = '02aa';
...
logFile = [DIR_LOG_FILE '/' 'nc_remove_attribute_' datestr(now, 'yyyymmddTHHMMSS') '.log'];
diary(logFile);
tic;
...
if (isempty(dataToProcessDir))
   ...
   for dirNum = 1:length(dirInfo)
      if ~(strcmp(dirInfo(dirNum).name, '.') || strcmp(dirInfo(dirNum).name, '..'))
         ...
         nc_remove_attribute_file([DIR_INPUT_NC_FILES '/' dirName]);
      end
   end
else
   % convert the data of this deployment
   nc_remove_attribute_file(dataToProcessDir);
end
...
%
% SYNTAX :
%  nc_remove_attribute_file(a_dirName)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   08/17/2021 - RNU - creation
% ------------------------------------------------------------------------------
function nc_remove_attribute_file(a_dirName)
...
for idF = 1:length(ncFiles)
   ...
   for idVar = 0:nbVars-1
      ...
      for idAtt = 0:nbAtts-1
         ...
         if (ismember(attName, [{'valid_min'} {'valid_max'}]))
         ...
         for idVar = 0:nbVars-1
            ...
            for idAtt = 0:nbAtts-1
               ...
               if (ismember(attName, [{'valid_min'} {'valid_max'}]))
                  attValue = netcdf.getAtt(fCdf, netcdf.inqVarID(fCdf, varName), attName);
                  if (isempty(attValue))
                     fprintf('   - Removing empty att ''%s'' of var ''%s''\n', ...
                        attName, varName);
                     netcdf.delAtt(fCdf, netcdf.inqVarID(fCdf, varName), attName);
